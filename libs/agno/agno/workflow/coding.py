...
class CodingWorkflow(Workflow):
    ...
    def __init__(self, name: str = "coding_workflow", workspace_path: str = "/app/workspace", **kwargs):
    ...
    def run(self, manifest: JobManifest) -> str:
        ...
        while implementation_attempt <= max_attempts:
            ...
            if validation_passed:
                ...
                val_response = validator.run(
                    f"Validate the changes for task: {manifest.task}\nCoder response: {coder_response.content}"
                )
